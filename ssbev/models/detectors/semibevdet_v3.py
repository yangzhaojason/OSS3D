...
from mmdet3d.models.bevdet_occ import BEVDet_Occ

@DETECTORS.register_module()
class semibevdet_v3(BEVDet):
    ...
    def forward_train(self,
                      ...
                      **kwargs):
        return super(semibevdet_v3, self).forward_train(
            points,
            img_metas,
            gt_bboxes_3d,
            gt_labels_3d,
...
@DETECTORS.register_module()
class semibevdet_occ_v3(BEVDet_Occ):
    ...
    def extract_feat_lowlevelfeatures(self, points, img, img_metas, get_lowlevelfeats=False, **kwargs):
        ...
        if get_lowlevelfeats:
            ...
            # for semi
            # 如果voxel_semantics是列表
            if isinstance(voxel_semantics, list):
                # 合并
                voxel_semantics = torch.stack(voxel_semantics, dim=0)      
            
            voxel_semantics = voxel_semantics.permute(0, 2, 1, 3) # 对齐特征
            img_feats = self.occ_feat(voxel_semantics, img_feats)
            return ([img_feats], pts_feats, depth, low_level_features, image_feature)
        else:
            img_feats, depth = self.extract_img_feat(img, img_metas, **kwargs)
            pts_feats = None
    ...
    def forward_train(self,
                      ...
                      gt_bboxes_ignore=None,
                      **kwargs):
        return super(semibevdet_occ_v3, self).forward_train(
            points,
            img_metas,
            gt_bboxes_3d,
            gt_labels_3d,
...
@DETECTORS.register_module()
class semibevdepth_v3(semibevdet_v3):
...
@DETECTORS.register_module()
class semibevdepth_occ_v3(semibevdet_occ_v3):
