...
class semi_bevdet_gs_v3(semi_detector):
    def __init__(self, model, train_cfg=None, test_cfg=None):
        super(semi_bevdet_gs_v3, self).__init__(
    ...
    def forward_train(self, img_inputs=None, img_metas=None, **kwargs):
        super(semi_bevdet_gs_v3, self).forward_train(img_inputs, img_metas, **kwargs)
        ...
        if 'voxel_semantics' in kwargs:
            voxel_semantics = kwargs.get('voxel_semantics')
        else:
        ...
        for idx, img_meta in enumerate(img_metas):
            ...
            if tag not in format_data.keys():
                ...
                if voxel_semantics is not None:
                    format_data[tag]['voxel_semantics'] = [voxel_semantics[idx]]
                ...
                if voxel_semantics is not None:
                    format_data[tag]['voxel_semantics'].append(voxel_semantics[idx])
                for ind, key in enumerate(matrix_name):
    ...
    def extract_student_info(self, student_data):
        student_info={}
        # 如果teacher_data有'voxel_semantics'的key
        if 'voxel_semantics' in student_data:
            voxel_semantics = student_data['voxel_semantics']
        else:
            voxel_semantics = None
    ...
    def extract_teacher_info(self, teacher_data):
        teacher_info={}
        # 如果teacher_data有'voxel_semantics'的key
        if 'voxel_semantics' in teacher_data:
            voxel_semantics = teacher_data['voxel_semantics']
        else:
            voxel_semantics = None
...
def calc_hwc(corners_box, centers_box, camera2lidar, camera2img):
    ...
    valid = (points_camera_centers[:, 2] > 0.5)
...
def images_feature_loss(gt_boxes, img_imputs, img_metas, bda_params, student_image_feature, teacher_image_feature):
    ...
    for i in range(len(imgs_path)):
        ...
        boxes_img_h, boxes_img_w, boxes_centers = calc_boxes_imghwc(boxes.corners.numpy(), boxes.gravity_center.numpy(), sensor2keyegos[0][i], intrins[0][i], img.shape[0], img.shape[1])
        ...
        if boxes_img_h.shape[0] > 0:
            ...
            aug_gaussian_mask_image = cv2.warpAffine(gaussian_mask_image, transformation_matrix, (img_metas[0]['batch_input_shape'][1], img_metas[0]['batch_input_shape'][0]))
            
            # 然后resize到特征图的大小
            target_size = (student_image_feature[0].shape[-1], student_image_feature[0].shape[-2])
            ...
            gaussian_mask_= torch.from_numpy(resize_aug_gaussian_mask_image).cuda()
            ...
            #     os.mkdir('tmp')
            # cv2.imwrite('tmp/{}_image_input.jpg'.format(i), imgs[0][i].cpu().detach().numpy().transpose(1, 2, 0) * 255)
            # cv2.imwrite('tmp/{}_mask_resize.jpg'.format(i), gaussian_mask_.cpu().detach().numpy() * 255)
