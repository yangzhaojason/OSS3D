...
class semibevdet_v1(BEVDet):
    
    def extract_feat_lowlevelfeatures(self, points, img, img_metas, get_lowlevelfeats=False, **kwargs):
        """Extract features from images and points."""
        if get_lowlevelfeats:
            img_feats, low_level_features, depth = self.extract_img_feat_list(img, img_metas, **kwargs)
            pts_feats = None
            return (img_feats, pts_feats, depth, low_level_features)
        else:
    ...
    def forward_train(self,
                      ...
                      **kwargs):
        return super(semibevdet_v1, self).forward_train(
            points,
            img_metas,
            gt_bboxes_3d,
            gt_labels_3d,
...
@DETECTORS.register_module()
class semibevdepth(semibevdet_v1):
