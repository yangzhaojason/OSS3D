...
class semi_bevdet_gs_v3_occ(semi_detector):
    def __init__(self, model, train_cfg=None, test_cfg=None):
        super(semi_bevdet_gs_v3_occ, self).__init__(
    ...
    def forward_train(self, img_inputs=None, img_metas=None, **kwargs):
        super(semi_bevdet_gs_v3_occ, self).forward_train(img_inputs, img_metas, **kwargs)
    ...
    def extract_student_info(self, student_data):
        ...
        if 'voxel_semantics' in student_data:
            ...
            voxel_semantics = None
        voxel_semantics_np = voxel_semantics[0].cpu().numpy()  # 假设 voxel_semantics[0] 是一个 CUDA 张量，需要先移到 CPU 上
        voxel_semantics_np = add_noise_and_mask(voxel_semantics_np)
        voxel_semantics = [torch.tensor(voxel_semantics_np).cuda()]
        ...
        img_feats, _, depth, low_features, image_feature = self.student.extract_feat_lowlevelfeatures(points=None, 
                                            img=student_data['img_inputs'], 
                                            img_metas=student_data['img_metas'],
                                            get_lowlevelfeats=True,
                                            voxel_semantics=voxel_semantics)
...
def add_noise_and_mask(tensor, noise_scale=0.1, mask_prob=0.95):
    # 添加随机噪声
    noise = np.random.normal(loc=0.0, scale=noise_scale, size=tensor.shape)
    tensor_with_noise = tensor + noise

    # 添加随机掩码
    mask = np.random.choice([0, 1], size=tensor.shape, p=[1-mask_prob, mask_prob])
    tensor_with_mask = tensor_with_noise * mask
